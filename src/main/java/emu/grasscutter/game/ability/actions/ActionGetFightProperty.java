    ...
    public boolean execute(
        ...
        if (Math.abs(fightPropertyValue) < 0.0001f && action.fightProp != null) {
            try {
                ...
                if (name != null && properties.containsKey(name)) fightPropertyValue = properties.getFloat(name);
            } catch (Throwable ignored) {}
            if (Math.abs(fightPropertyValue) < 0.0001f) {
                fightPropertyValue = owner.getFightProperty(FightProperty.FIGHT_PROP_CUR_SPECIAL_ENERGY);
            }
        }
        ...
        if (resolvedTarget.getScene() != null && resolvedTarget.getScene().getHost() != null) {
            resolvedTarget.getScene().getHost().sendPacket(
                    new PacketServerGlobalValueChangeNotify(resolvedTarget, action.globalValueKey, fightPropertyValue));
        }
