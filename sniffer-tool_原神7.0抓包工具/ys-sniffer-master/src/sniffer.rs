    ...
    if data.len() == 20 {
        ...
        match opcode {
            ...
            CONV_CMD => {
                ...
                info!("Received conversation ID from server: ({}, {})", conv, token);
            },
            DISCONNECT_CMD => {
                trace!("Disconnect handshake operation received from {:?}.", source);
            },
            _ => warn!("Unknown handshake operation received: {:x?} (data: {:02x?})", opcode, data)
        }
    } else {
...
pub struct Processor {
    ...
    // Diagnostics: throttled drop counter.
...
impl Processor {
    ...
    /// Logs a throttled warning about packets dropped while decrypting.
    fn report_drop(&mut self) {
        self.dropped += 1;

        let now = Instant::now();
        let due = match self.last_report {
            Some(last) => now.duration_since(last).as_secs() >= 3,
            None => true
        };

        if due {
            warn!(
                "{} packets dropped: no usable encryption key/handshake data yet",
                self.dropped
            );
        ...
        match self.key {
            PacketKey::None => {
                ...
                let key = match DISPATCH_KEYS.get(&index) {
                    ...
                    None => {
                        self.report_drop();
                        return Err(anyhow!("No key found for index: {}", index))
                    }
                };
            ...
            PacketKey::Dispatch(ref key) => {
                ...
                let (sent_time, seed) = match &self.handshake {
                    ...
                    None => {
                        self.report_drop();
                        return Err(anyhow!("No handshake data available."))
                    }
                };
                ...
                match bruteforce(&self.known_seeds, sent_time, seed, &data) {
                    Some(seed) => {
                        ...
                        match key.xor_or(&mut data) {
                            Ok(_) => {
                                info!("Encryption key found for session! Seed: {}", seed);
                                Ok(data)
                            },
                            Err(_) => {
                    ...
                    None => {
                        self.report_drop();
                        Err(anyhow!("Unable to find the encryption key seed."))
                    }
                }
            }
        }
