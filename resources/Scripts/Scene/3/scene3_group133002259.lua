...
suites = {
	...
	},
	{
		...
		triggers = { "ENTER_REGION_259002", "GROUP_LOAD_259003", "GROUP_REFRESH_259004", "ENTER_REGION_259007", "LEAVE_REGION_259008", "GROUP_WILL_UNLOAD_259010", "TIME_AXIS_PASS_259011", "SELECT_OPTION_359" },
		rand_weight = 100
	},
	{
		-- suite_id = 5,
...
function action_EVENT_ANY_MONSTER_DIE_344(context, evt)
	-- 击杀后一律关试炼选项、清二阶段场地；不要 AddExtraGroupSuite(5)（会死动画时刷出「开启试炼」）
	ScriptLib.SetGroupVariableValue(context, "boss_exist", 0)
	ScriptLib.DelWorktopOptionByGroupId(context, 133002259, 259009, 2902)
	ScriptLib.DelWorktopOptionByGroupId(context, 133002259, 2299, 2902)
...
function condition_EVENT_SELECT_OPTION_359(context, evt)
	-- 入口操作台 2299 或剑上操作台 259009，option 2902=开启试炼
	if 2299 ~= evt.param1 and 259009 ~= evt.param1 then
		return false
	end
...
-- 剑上 259009 也挂开启试炼选项（客户端交互点在剑，入口台 2299 离剑约 35m）
-- 征讨之花存在或 boss 已死（boss_exist=0）时不要挂，避免死亡动画期间出现「开启试炼」
local function ensure_andrius_start_options(context)
	if ScriptLib.GetGroupVariableValue(context, "boss_exist") ~= 1 then
		ScriptLib.DelWorktopOptionByGroupId(context, 133002259, 259009, 2902)
		ScriptLib.DelWorktopOptionByGroupId(context, 133002259, 2299, 2902)
		return
	end
	if ScriptLib.CheckIsInGroup(context, 133002259, 259005) then
		ScriptLib.DelWorktopOptionByGroupId(context, 133002259, 259009, 2902)
		ScriptLib.DelWorktopOptionByGroupId(context, 133002259, 2299, 2902)
		return
	end
	ScriptLib.SetWorktopOptionsByGroupId(context, 133002259, 259009, {2902})
	ScriptLib.SetWorktopOptionsByGroupId(context, 133002259, 2299, {2902})
end
...
function action_EVENT_GROUP_LOAD_259003(context, evt)
	...
	if 0 ~= ScriptLib.GetGroupVariableValue(context, "boss_exist") then
		...
		ensure_andrius_start_options(context)
	end
	return 0
end
