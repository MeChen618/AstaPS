# -*- coding: utf-8 -*-
"""Compile Cryo Traveler ice-infusion embryo fix and inject into LunaGC-7.0.0.jar."""
...
OUT = ROOT / "_ice_embryo_classes"
LOMBOK = ROOT / "lombok-1.18.30.jar"
ERR_LOG = ROOT / "_ice_embryo_javac_err.txt"
JAVA = Path(r"C:\Program Files\Microsoft\jdk-17.0.20.101-hotspot\bin\javac.exe")

files = [
    "src/main/java/emu/grasscutter/game/ability/IceTravelerAbilityHelper.java",
...
def main() -> int:
    ...
    if r.returncode != 0:
        sys.stderr.buffer.write(b"javac failed; see _ice_embryo_javac_err.txt\n")
        return r.returncode
    ...
    bak = ROOT / f"LunaGC-7.0.0.jar.bak-before-ice-embryo-{stamp}"
    shutil.copy2(JAR, bak)

    tmp = ROOT / "LunaGC-7.0.0.jar.tmp-ice-embryo"
    if tmp.exists():
        tmp.unlink()
    seen = set()
    ...
    summary = [f"bak={bak.name}", f"classes={len(adds)}"]
    summary.extend("  " + rel for rel in sorted(adds))
    (ROOT / "_ice_embryo_inject_ok.txt").write_text("\n".join(summary), encoding="utf-8")
