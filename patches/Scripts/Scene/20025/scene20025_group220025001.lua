...
triggers = {
	...
	{ config_id = 1000003, name = "ENTER_REGION_3", event = EventType.EVENT_ENTER_REGION, source = "", condition = "condition_EVENT_ENTER_REGION_3", action = "action_EVENT_ENTER_REGION_3", trigger_count = 1, forbid_guest = false },
...
-- 变量
variables = {
	{ config_id = 1, name = "dvalin_done", value = 0, no_refresh = false }
}
...
function condition_EVENT_ENTER_REGION_3(context, evt)
	...
	-- 结算后传送回领奖台会再次进区，禁止重复刷龙（否则会出现半透明龙+BOSS NAME血条）
	if ScriptLib.GetGroupVariableValue(context, "dvalin_done") == 1 then
		return false
	end
...
function action_EVENT_DUNGEON_SETTLE_1012(context, evt)
	-- 标记已结算，阻止领奖台区域再次 CreateMonster
	ScriptLib.SetGroupVariableValue(context, "dvalin_done", 1)
	ScriptLib.RemoveEntityByConfigId(context, 220025001, EntityType.MONSTER, 1001)
